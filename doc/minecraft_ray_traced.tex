...
Function parameters are passed as the following\footnote{\url{https://docs.oracle.com/javase/8/docs/technotes/guides/jni/spec/types.html}}:
...
We start off by creating a class that implements the \texttt{IFMLLoadingPlugin} interface.
This interface has five methods, but we only need to implement the \texttt{getASMTransformerClass} method.
This method returns an array of classes that implement the \texttt{IClassTransformer} interface.
We create a class that implements this interface and put that in the array.

The \texttt{IClassTransformer} interface only has one method: \texttt{transform}.
This function takes three parameters:
\begin{itemize}
  \item The name of the class to be transformed
  \item The deobfuscated name of the class. Unused during development.
  \item The bytecode of the class
\end{itemize}

\begin{itemize}
  \item \texttt{EntityRenderer} \\
%  \item \texttt{GuiVideoSettings} \\
...
% TODO talk about vertex buffer grid
...
\end{equation}
This maps the player's position to $[0, 1)$.
...
\begin{figure}
  ...
  S& = \frac{Q + R}{2} \nonumber \\
  \text{\faVideoCamera}_{pos}& = S + \frac{\vec{u}}{\lvert\lvert\vec{u}\rvert\rvert}\bigcdot\frac{\lvert\lvert\overrightarrow{PR}\rvert\rvert}{2 \tan\left(\theta\right)} \label{eq:rayorigin}
  \end{align}
